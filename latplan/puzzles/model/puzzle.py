...
def generate(configs, width, height, custom, **kwargs):
    load(width, height)
    ...
    def build():
        ...
        if not custom:
            matches = K.permute_dimensions(configs_one_hot, [0,2,1]) # USE for DEFAULT dataset
        else:
            matches = K.permute_dimensions(configs_one_hot, [0,1,2]) # USE FOR CUSTOM DATASET (well... this fn is not used actually)
...
def states(width, height, configs=None, custom=False, **kwargs):
    digit = width * height
    if configs is None:
        configs = generate_configs(digit)
    return generate(configs,width,height,custom=custom, **kwargs)
