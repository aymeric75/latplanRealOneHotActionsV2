...
Poor python coders cannot enjoy the cleanness of CLOS :before, :after, :around methods."""
 ...
 This method may be overloaded by the subclass into a specific training method, e.g. GAN training."""
        ...
        if self.parameters["load_sae_weights"]:

            pathh = "samples/puzzle_mnist_3_3_5000_CubeSpaceAE_AMA4Conv_kltune2/logs/05-06T11:21:55.052/"

            for layer in self.nets[0].layers:
                if layer.name in SAE_layers:
                    weights = layer.get_weights()
                    #np.save(pathh+layer.name+"_weights.npy", weights)

                    loaded_weights = np.load(pathh+layer.name+"_weights.npy", allow_pickle=True)
        ...
        #     wandb.define_metric("loss", summary="min", step_metric="epoch")
        #     wandb.define_metric("tau", summary="min", step_metric="epoch")
        #     wandb.define_metric("pdiff_z1z2", summary="min", step_metric="epoch")
        #     wandb.define_metric("pdiff_z0z3", summary="min", step_metric="epoch")
        #     wandb.define_metric("pdiff_z0z1", summary="min", step_metric="epoch")
        #     wandb.define_metric("pdiff_z0z2", summary="min", step_metric="epoch")
        #     wandb.define_metric("kl_z0", summary="min", step_metric="epoch")
        #     wandb.define_metric("kl_z1", summary="min", step_metric="epoch")
        #     wandb.define_metric("kl_a_z0", summary="min", step_metric="epoch")
        #     wandb.define_metric("kl_a_z1", summary="min", step_metric="epoch")
        #     wandb.define_metric("kl_z1z2", summary="min", step_metric="epoch")
        #     wandb.define_metric("kl_z0z3", summary="min", step_metric="epoch")
        #     wandb.define_metric("x0y0", summary="min", step_metric="epoch")
        #     wandb.define_metric("x1y1", summary="min", step_metric="epoch")
        #     wandb.define_metric("x0y3", summary="min", step_metric="epoch")
        #     wandb.define_metric("x1y2", summary="min", step_metric="epoch")
        #     wandb.define_metric("elbo", summary="min", step_metric="epoch")
        ...
        def generate_logs(data, data_to, epoch=0, forwandb=False):
            ...
            # print(actions_array[index])

            # images_array = np.expand_dims(images_array[index], axis=0)
            # actions_array = np.expand_dims(actions_array[index], axis=0)
            ...
            # 2) ici evalue sur le 1er élément, puis sur les 2 premiers POUR vérifier
            ...
            for epoch in range(start_epoch,start_epoch+epoch):
                ...
                for train_subdata_cache, train_subdata_to_cache in zip(train_data_cache,train_data_to_cache):
                    ...
                    for item in train_subdata_cache:
                        ...
                        # row = np.tile(item[1], 2) [ CODE for if one Input ]
                        # matrix = np.tile(row, (48, 1))
