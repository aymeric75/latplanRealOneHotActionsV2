...
def main():
    ...
    with timers.timing("Parsing", True):
        task = pddl_pars.open(
            domain_filename=options.domain, task_filename=options.task) # options.task
    print("hh2")

    #print(str(task.requirements))
    task.requirements = str(task.requirements).replace(",","").split(" ")
    # print("")
    # print(task.actions[0].precondition.parts[0])
    # print("ok")
    ...
    #########################################################################################
    # 'remove_duplicates' Foreach action REMOVE the DUPLICATES BETWEEN PRECONDS and effects
    #########################################################################################
    if("--operation=remove_duplicates" == sys.argv [-1]):
    ...
    #print(task.get_pddl_problem())

    if("--operation=remove_not_from_prob" == sys.argv [-1]):
...
if __name__ == "__main__":
    ...
    #print(sys.argv [-1])
    print(sys.argv)
    #assert("--operation=remove_not_from_prob" == sys.argv [-1])
    #assert("--operation=remove_duplicates" == sys.argv [-1])
